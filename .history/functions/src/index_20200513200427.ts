...
exports.tripCompletado = functions.database.ref('asignados/{idRepartidor}/{idPedido}')
 .onUpdate(async (change, context) => {
     const idRepartidor = context.params.idRepartidor
     const idPedido = context.params.idPedido
     const date = await formatDate()
     ...
     if (!before.terminado && after.terminado) {
         return admin.database().ref(`usuarios/${after.cliente.uid}/pedidos/activos/${idPedido}/terminado`).set(after.terminado)
         .then(() => admin.database().ref(`pedidos/activos/${after.negocio.idNegocio}/detalles/${idPedido}/terminado`).set(after.terminado))
         .then(() => admin.database().ref(`pedidos/historial/por_fecha/${date}/${idPedido}/terminado`).set(after.terminado))
