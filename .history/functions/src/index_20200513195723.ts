...
exports.tripCompletado = functions.database.ref('asignados/{idRepartidor}')
 .onUpdate(async (change, context) => {
     const idRepartidor = context.params.idRepartidor
     const date = await formatDate()
     ...
     if (!before.terminado && after.terminado) {
         return admin.database().ref(`usuarios/${after.cliente.uid}/pedidos/activos/${after.id}/terminado`).set(after.terminado)
         .then(() => admin.database().ref(`asignados/${idRepartidor}/${after.id}`).remove())
         .then(() => )
     }
 })
