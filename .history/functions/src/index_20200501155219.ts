...
exports.solicitaRepartidor = functions.database.ref('pedidos/repartidor_pendiente/{idNegocio}/{idPedido}')
    .onCreate(async (snapshot, context) => {
        ...
        .then(async (repartidores) => {
            ...
            if (repartidores[0].token) return sendFCM(repartidores[0].token, 'Tienes un nuevo pedido', pedido.id)
            else {
                const error = 'no_token_registered'
                throw error
            }
        })
        .then(() => admin.database().ref(`repartidores_asociados_data/${pedido.region}/${repartidorId}/last_notification`).set(Date.now()))
        .catch(err => {
            // Alertar a Admin
            console.log(err)
        })
...
function sendFCM(token: string, mensaje: string, pedidoID?: string) {
