...
exports.solicitaRepartidor = functions.database.ref('pedidos/repartidor_pendiente/{uid}/{idPedido}')
    .onCreate(async (snapshot, context) => {
        const idPedido = context.params.idPedido
        const pedido: Pedido = snapshot.val()
        console.log(pedido);
        // Get repartidores asociados
        return admin.database().ref(`repartidores_asociados_data/${pedido.region}`).once('value')
        ...
        .then(async (repartidores) => {
            ...
            return sendFCM(repartidores[0].token, 'Tienes un nuevo pedido')
        })
        .then(() => admin.database().ref(`pedidos/repartidor_pendiente/${idPedido}`).remove())
        .catch(err => {
            // Alertar a Admin
            console.log(err)
        })
