...
exports.solicitaRepartidor = functions.database.ref('pedidos/repartidor_pendiente/{idNegocio}/{idPedido}')
    .onCreate(async (snapshot, context) => {
        ...
        .then(async (repartidores_raw) => {
            const repartidores = repartidores_raw.filter(r => r.token)
        ...
        sendFCMPedido(repartidores[0].token, 'Tienes un nuevo pedido', pedido, stampNotification)
        .then(() => admin.database().ref(`repartidores_asociados_data/${pedido.region}/${repartidorId}/last_notification`).set(stampNotification))
        .catch(err => {
            // Alertar a Admin
            console.log(err)
        })
...
function sendFCMPedido(token: string, mensaje: string, pedido: Pedido, notificado: number) {
    const payload: admin.messaging.MessagingPayload = {
        notification: {
            title: 'Spot',
            body: mensaje,
        ...
        data: {
            ...
            notificado: notificado.toString()
        }
      };
      const options = {
          priority: "high"
