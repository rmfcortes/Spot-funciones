...
exports.pedidoAceptado = functions.database.ref('pedidos/activos/{idNegocio}/detalles/{idPedido}')
    .onUpdate(async (change, context) => {
        ...
        await admin.database().ref(`usuarios/${idCliente}/pedidos/activos/${idPedido}/entrega`).set(after.entrega);
        return admin.database().ref(`usuarios/${idCliente}/token`).once('value')
        .then((data: any) => {
            const token = data.val();
            if (token) {
...
exports.nuevoRepartidor = functions.database.ref('nuevo_repartidor_externo/{region}/{idRepartidor}')
    .onCreate(async (snapshot, context) => {
        const idRepartidor = context.params.idRepartidor
        const region = context.params.region
        const repartidor: RepartidorInfo = snapshot.val()
        try {
            ...
            await admin.database().ref(`repartidores_asociados_info/${region}/detalles/${repartidor.preview.id}`).set(repartidor.detalles)
            await admin.database().ref(`repartidores_asociados_info/${region}/preview/${repartidor.preview.id}`).set(repartidor.preview)
...
exports.solicitaRepartidor = functions.database.ref('pedidos/repartidor_pendiente/{idNegocio}/{idPedido}')
    .onCreate(async (snapshot, context) => {
        ...
        .then(() => admin.database().ref(`repartidores_asociados_data/${pedido.region}`).once('value'))
        ...
        .then(() => admin.database().ref(`pedidos/historial/${date}/${idPedido}`).once('value'))
        ...
        .then(() => admin.database().ref(`repartidores_asociados_data/${pedido.region}/${repartidorId}/last_notification`).set(pedido.last_solicitud))
        .then(() => pedido.pendiente_repartidor = true)
        .then(() => admin.database().ref(`pedidos/historial/por_fecha/${date}/${idPedido}`).update(pedido))
        .catch(err => console.log(err))
    })
...
exports.pedidoTomadoRepartidor = functions.database.ref('pendientes_aceptacion/{idRepartidor}/{idPedido}')
    .onCreate(async (snapshot, context) => {
        ...
        let pedido: Pedido
        const date = await formatDate()
        return admin.database().ref(`pedidos/historial/por_fecha/${date}/${idPedido}`).once('value')
        ...
        .then(async (pedido_raw: Pedido) => {
            ...
            if (pedido && pedido.repartidor) {
                ...
                throw admin.database().ref(`pendientes_aceptacion/${idRepartidor}/${idPedido}`).set('El pedido ha sido tomado por otro repartidor')
            }
            return admin.database().ref(`repartidores_asociados_info/${idRepartidor}`).once('value')
            .then(repartidorVal => repartidorVal ? repartidorVal.val() : null)
            .then((repartidor: Repartidor) => {
                pedido.repartidor = repartidor
                return admin.database().ref(`usuarios/${pedido.cliente.uid}/pedidos/activos/${pedido.id}/repartidor`).set(repartidor)
            })
            .then(() => admin.database().ref(`asignados/${idRepartidor}/${pedido.id}`).set(pedido))
            .then(() => admin.database().ref(`repartidores_asociados_data/${pedido.region}/${idRepartidor}/last_pedido`).set(Date.now()))
            .then(() => {
                pedido.pendiente_repartidor = false
                return admin.database().ref(`pedidos/historial/por_fecha/${date}/${idPedido}`).update(pedido)
            })
            .then(() => admin.database().ref(`pedidos/activos/${pedido.negocio.idNegocio}/detalles/${pedido.id}`).update(pedido))
            .then(() => admin.database().ref(`notifications/${pedido.last_notificado}/${idPedido}`).remove())
        })
    })

exports.tripCompletado = functions.database.ref('asignados/{idRepartidor}/{idPedido}/entregado')
 .onUpdate(async (change, context) => {
     const idRepartidor = context.params.idRepartidor
     const idPedido = context.params.idPedido
     const date = await formatDate()
     ...
     if (!before.entregado && after.entregado) {
         return admin.database().ref(`usuarios/${after.cliente.uid}/pedidos/activos/${idPedido}/entregado`).set(after.entregado)
         .then(() => admin.database().ref(`pedidos/activos/${after.negocio.idNegocio}/detalles/${idPedido}/entregado`).set(after.entregado))
         .then(() => admin.database().ref(`pedidos/historial/por_fecha/${date}/${idPedido}/entregado`).set(after.entregado))
         .then(() => admin.database().ref(`pedidos/historial/por_repartidor/${idRepartidor}/${date}/${idPedido}`).set(after))
         .then(() => admin.database().ref(`pedidos/historial/por_negocio/${after.negocio.idNegocio}/${date}/${idPedido}`).set(after))
         .catch(err => console.log(err))
     }
     return null
 })
...
function formatDate() {
    return new Promise((resolve, reject) => {        
        const dMX = new Date().toLocaleString("en-US", {timeZone: "America/Mexico_City"})
...
export interface RepartidorExternoPreview {
    foto: string;
    id: string;
    nombre: string;
    telefono: string;
...
export interface RepartidorInfo {
    preview: RepartidorExternoPreview;
